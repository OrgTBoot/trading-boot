...
public class ETFStrategyV2 extends AbstractStrategyDefinition {
    ...
    private Strategy initStrategy() {
        ...
        DoubleEMAIndicator demaShortIndicator = new DoubleEMAIndicator(closePrice, params.getShortBarCount());
        DoubleEMAIndicator demaLongIndicator = new DoubleEMAIndicator(closePrice, params.getLongBarCount());
        ...
        AwesomeOscillatorIndicator awesomeOscillatorInd = new AwesomeOscillatorIndicator(series);
        DistanceFromMAIndicator distanceFromLongDEMAInd = new DistanceFromMAIndicator(series, demaLongIndicator);
        //SHARED RULES
        ...
        //ENTRY RULES
//        Rule awesomeOscBuy = debug(new OverIndicatorRule(awesomeOscillatorInd, 0));
        Rule awesomeOscSell = debug(new UnderIndicatorRule(awesomeOscillatorInd, 0));
//        Rule crossDEMABuy = debug(new CrossedUpIndicatorRule(demaShortIndicator, demaLongIndicator));
        Rule priceOverLongDEMABuy = debug(new OverIndicatorRule(closePrice, demaLongIndicator));
        Rule crossDEMAsBuy = debug(new CrossedUpIndicatorRule(demaShortIndicator, demaLongIndicator), "DEMA CROSS BUY");
        Rule crossDEMASell = debug(new CrossedDownIndicatorRule(demaShortIndicator, demaLongIndicator), "DEMA CROSS SELL");
        Rule distanceDemaBuy = debug(new OverIndicatorRule(distanceFromLongDEMAInd, 0.01), "DEMA DIST BUY");
        Rule distanceDemaSell = debug(new UnderIndicatorRule(distanceFromLongDEMAInd, 0.001), "DEMA DIST SELL");
        ...
        Rule preMarketHours = debug(new MarketPreHoursRule(series), "PRE MKT HOURS");
...
//                        .and(lastExit30BarsAgo)
        ...
        //EXIT RULES
        Rule priceUnderDEMASell = debug(new UnderIndicatorRule(closePrice, demaLongIndicator), "DEMA cross Down");
        ...
        Rule downTrendLoss05 = debug(loss05Percent.and(crossDEMASell).and(chandelierOverPriceSell), "DT 0.5% LOSS");
